...
육각형 타일로 구성된 맵 위에서 한 줄로 몰려오는 적을 막아내는 클래식 타워 디펜스 게임이다. 플레이어는 제한된 자원으로 타워를 배치하고 업그레이드하며, 웨이브마다 강해지는 적을 최대한 오래 버티는 것이 목표다.

## 핵심 게임 플레이

- **타워 배치와 업그레이드**: 육각 격자 타일 위에 타워를 짓고, 자원을 투자해 공격력·사거리·공격 속도를 강화한다.
- **웨이브 방어**: 정해진 경로를 따라 이동하는 적을 처치해 기지를 지키고, 웨이브를 돌파할수록 보상이 증가한다.
- **자원 관리**: 적 처치 보상과 웨이브 보너스를 활용해 다음 방어 준비에 투자한다.

## 배경음악

- **인트로/아웃트로**: `start-272637` 트랙을 메인 메뉴와 결과 화면에 사용한다. 게임 분위기를 차분히 열고 닫는 용도로 길이와 볼륨을 맞춰 배치한다.
- **게임 플레이**: `warrior-defense-fighting-music-335681` 트랙을 플레이 중 루프 재생한다. 효과음과 겹치지 않도록 -6dB 정도로 밸런스를 맞추고, 웨이브 시작·클리어 순간에는 페이드 인/아웃을 적용한다.
