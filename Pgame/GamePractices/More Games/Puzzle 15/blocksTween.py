...
# 16-blocks
# added block tweening and easing
...
class Board:
    ...
    def moveBlock(self, index):
        ...
        if areAdjacent(index, blankIdx):  # can move
            # print("Moving", self.blocks[index].val, "from", index, "to", blankIdx)

            self.blocks[index].animator = \
                  Animator(easing.easeInOutCubic, toCoord(index), toCoord(blankIdx), FPS/2)  # quite fast
                         # easing.linear

            isTweening = True    # tweening initiated by the board

            # swap block and the blank
...
class Animator:
    # an animation between beginCoord and endCoord,
    # spread over numFrames calls, using an easing function (easeFn)
    # imported from easing.py
    # see Robert Penner's Easing Functions (http://robertpenner.com/easing/)

    def __init__(self, easeFn, beginCoord, endCoord, numFrames):
        self.easeFn = easeFn
        self.beginCoord = beginCoord
        self.endCoord = endCoord
        self.numFrames = numFrames
        # print("  Animating", beginCoord, "to", endCoord, "over", numFrames, "frames")
        self.currFrame = 0
        self.change = [endCoord[0]-beginCoord[0], endCoord[1]-beginCoord[1]]


    def isEnded(self):
        return (self.currFrame == self.numFrames)


    def getNextCoord(self):
        if self.currFrame == self.numFrames-1:   # last frame
            self.currFrame += 1
            return self.endCoord  # avoids any inaccuracies in adding steps
        elif self.currFrame < self.numFrames:
            x = self.easeFn(self.currFrame, self.beginCoord[0], self.change[0], self.numFrames)
            y = self.easeFn(self.currFrame, self.beginCoord[1], self.change[1], self.numFrames)
                   # apply the easing function to both the x- and y- movements
                   # use frame count as the 'time' units
            self.currFrame += 1
            return [x,y]
...
class Block(pygame.sprite.Sprite):
    ...
    def __init__(self, i, j):
        ...
        self.animator = None   # new -- stores the animator object
    ...
    def draw(self, screen, idx):
        ...
        if self.animator is not None:
            coord = self.animator.getNextCoord()
            # print(coord)
            if self.animator.isEnded():
                self.animator = None
                isTweening = False    # tweening switched off by the block
        else:
            coord = toCoord(idx)
...
clicks = 0
gameOver = False
isTweening = False     # new -- required for tweening and easing
...
while running:
    ...
    # update game
    if not gameOver and not isTweening:
